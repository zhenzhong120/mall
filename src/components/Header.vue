<template>
  <header class="header">
    ...
    <div class="navbar">
      <div class="navbar-left-container">
        <a href="#">
          ...
          <div class="navbar-detail">
            <span class="navbar-brand-text">慕 课 网</span>
            <span class="navbar-web">imooc.com</span>
          </div>
        </a>
      </div>
      <div class="navbar-right-container" style="display: flex;">
        <div class="navbar-menu-container">
          ...
          <!-- <input type="text" v-if="nickName" :value="nickName"> -->
          <a
    ...
    <!-- 遮罩层用v-if,因为它的样式设置的是显示状态，所以用v-if 创建和销毁标签来控制它的显示和隐藏 -->
...
export default {
  data() {
    return {
      userName: "",
      userPwd: "",
      errorTip: false, //定义登录校验提示信息的显示和隐藏，默认隐藏
      loginModalFlag: false, //登录页面的显示和隐藏
      //nickName: "",
    };
  },
  computed: {
      ...
      this.axios
        ...
        .then((response) => {
          ...
          if (res.status == "0") {
            ...
            this.$store.commit("updateUserTnfo", res.result.userName);
            this.getCartCount();
          } else {
            this.erropTip = true; //登录失败显示校验信息
          }
        });
    },
    logOut() {
      this.axios.post("/users/logout").then((response) => {
        let res = response.data; //response.data获取接口返回的数据，插件自动封装的
        if (res.status == "0") {
          // this.nickName = "";
          this.$store.commit("updateUserTnfo", "");
        }
      });
    },
    checkLogin() {
      this.axios.get("/users/checkLogin").then((response) => {
        let res = response.data;
        if (res.status == "0") {
          this.$store.commit("updateUserTnfo", res.result);
          this.getCartCount(); //页面刷新的时候也调用一次购物车数量接口
